...
void CAN_WatchDog_Builtin(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (needReset)
        {
            ...
            {
                Serial.println("CAN_WatchDog_Builtin : need reset !!!!!!!!!!!!!");
                if (espCan->debuggingMode)
                    Serial.println("Builtin CAN Forced Reset!");
                CAN_stop();
                CAN_init();
            }
        }
...
void task_LowLevelRX(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        //  if(xQueueReceive(lowLevelRXQueue, &rxFrame, 1 /*portMAX_DELAY*/)==pdTRUE)
        esp_err_t ret = CAN_read_frame(rxFrame);
        if (ret == ESP_OK)
        {
            // Serial.println("A");
...
int ESP32CAN::_setFilter(uint32_t id, uint32_t mask, bool extended)
{
    ...
    if (debuggingMode)
        Serial.println("Could not set filter!");
    return -1;
}
...
{
    if (debuggingMode)
        Serial.println("Built in CAN Init");
    for (int i = 0; i < BI_NUM_FILTERS; i++)
    {
        filters[i].id = 0;
        filters[i].mask = 0;
...
bool ESP32CAN::processFrame(CAN_frame_t &frame)
{
    ...
    for (int i = 0; i < 8; i++)
        msg.data.byte[i] = frame.data.u8[i];
...
bool ESP32CAN::sendFrame(CAN_FRAME &txFrame)
{
    ...
    for (int i = 0; i < 8; i++)
        __TX_frame.data.u8[i] = txFrame.data.byte[i];
...
uint32_t ESP32CAN::get_rx_buff(CAN_FRAME &msg)
{
    ...
    if (xQueueReceive(CAN_cfg.rx_queue, &__RX_frame, 0) == pdTRUE)
    {
        ...
        for (int i = 0; i < 8; i++)
            msg.data.byte[i] = __RX_frame.data.u8[i];
        return true;
    }
    return false;
}

uint32_t ESP32CAN::isFaulted2()
{
    return CAN_isFaulted2();
}